...
class RejseplanenSource(BaseSource[BEvent]):

    def execute(self):
        last_seen = {}
        for t in self._fetch_trains():
            last_seen[t["id"]] = t["last_station"]

        while True:
            trains = self._fetch_trains()
            for t in trains:
                key = t["id"]
                if key in last_seen:
                    if last_seen[key] != t["last_station"]:
                        # print(f"Train {t['name']} changed from {last_seen[key]} to {t['last_station']}")
                        self.produce(self._to_b_event(t, last_seen[key]))
    ...
    def _to_b_event(self, train: dict, prev_station: str) -> BEvent:
        ...
        ev.event_attributes["train-delay"] = self._try_parse_int(train["delay"])
