...
control_flow = {
    "Order": heuristics_miner_lossy_counting(model_update_frequency=1),
    "Item": heuristics_miner_lossy_counting(model_update_frequency=1),
}
...
combined_log.pipe(
    oc_operator.op()
).subscribe(lambda model: print("[MODEL]", model))
